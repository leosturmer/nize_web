...
if (empty($_SESSION['pedidoSelecionado'])) {
  ...
  echo $_SESSION['msg'];
  header("location:/nize_web/visualizacao_pedidos");
  exit;
}
          ...
          foreach ($_SESSION['carrinho'] as $id_produto => $item) {
            ...
            if ($produtoVendido) {
              ...
              if (basename($_SERVER['PHP_SELF']) == 'alteracao_pedidos.php') {
                echo "<a href='/nize_web/php/controller/pedidoControle.php?op=removerQuantidade&id=$id_produto&id_pedido=$id_pedido' class='btn-remover'><span class='bi bi-x-square'>Remover</a>";
              }
          ...
          <div class="form-pedidos-items">
            <fieldset id="pedidos-form">
              ...
              <label for="comentarioPedido">
                Comentários
                <textarea maxlength="500" rows="5" cols="40" name="comentarioPedido" id="comentarioPedido" placeholder="Detalhes do pedido, dos produtos, da entrega, do cliente, entre outros."><?php echo $infoPedidoSession['comentario'] ?></textarea>
              </label>
